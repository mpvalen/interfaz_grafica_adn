...
def lector_dose_data_FLUKA(dose_data_path):
    ...
    for line in ascii_file:
        ...
        if "Z coordinate" in line:
            ...
            values += [float(i)
                       for i in list(filter(None, line.strip().split(' ')))]
        contador += 1
...
def fluka_to_data(spectrum_data_path, dose_data_path, database_path,
    ...
    # Lector de espectro nos entrega todos los espectros, leyendo lo de FLUKA
    Ebins, spectrum_avg, spectrum_std = lector_spectrum_data_FLUKA(
        spectrum_norm_path, detector_depths)
    filename = spectrum_norm_path.split(os.sep)[-1]

    if os.path.exists(output_folder):
        # path completo de una carpeta ya existente
        path_file = os.path.join(
            output_folder, f'proc_{filename}_{dose_norm_max}Gy')
    else:
        ...
        try:
            os.mkdir(folder)
            path_file = os.path.join(
                folder, f'proc_{filename}_{dose_norm_max}Gy')
    ...
    summarized_tab = []
    newtablis.write(
        "Profund[mm] D[A.U.]   D-Error    DSB-Yield  Yield-Error  Lambda  Lambda-Error    S      S-Error\n")
    ...
    for index, detector_depth in enumerate(detector_depths):
        ...
        if sum(spectrum_at_detector_depth) != 0:
            ...
            yerrave = np.average(
                yerr, weights=spectrum_at_detector_depth)*DNA/6
            lave = np.average(
                l, weights=spectrum_at_detector_depth)*5**2/NDIA**2
            lerrave = np.average(
                lerr, weights=spectrum_at_detector_depth)*5**2/NDIA**2
        else:
            yave = 0
            yerrave = 0
            lave = 0
        ...
        sumtabline = [detector_depth, interp_doses[index], interp_dosese[index],
                      yave, yerrave, lave, lerrave, S, Serr]
        # también se genera una tabla resumida, ponderando yields.
        summarized_tab.append(sumtabline)
    newtablis.close()
    # retorna exactamente lo que queremos saber, una tabla con profundidad, dosis, yield, lambda,
    return summarized_tab
