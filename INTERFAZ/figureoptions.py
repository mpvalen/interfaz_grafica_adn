...
# Modified to add fontsize settings 
...
def figure_edit(axes, parent=None):
    ...
    general = [('Title', axes.get_title()),
               ...
               ('Scale', [axes.get_yscale(), 'linear', 'log', 'logit']),
                sep,
               (None, '<b>Fontsize settings</b>'),
               ('Title Fontsize', int(axes.title.get_fontsize())),
               ('Label Fontsize', int(axes.xaxis.label.get_fontsize())),
               ('Tick Size', _ticksize),
               sep,
    ...
    legend = [(None, '<b>Legend settings</b>'),
              ('Legend fontsize', _fontsize)]



    otros = [
            ('Multiplicar eje X por factor', 1),
            ('Multiplicar eje Y por factor', 1)]
    # Save the unit data
    ...
    datalist = [(general, "Axes", ""), (legend, "Legend", ""), (otros, 'Otros', '')]
    if curves:
    ...
    def apply_callback(data):
        ...
        # Set / General
        (title, xmin, xmax, xlabel, xscale,
         ymin, ymax, ylabel, yscale, titlefontsize, labelsize, ticksize, generate_legend) = general 
        ...
        new_legend = axes.legend(fontsize=legend_fontsize)

        # Set Otros
        (xfactor, yfactor) = otros
        axes.set_xlim(xmin * xfactor, xmax * xfactor)
        axes.set_ylim(ymin * yfactor, ymax * yfactor)
