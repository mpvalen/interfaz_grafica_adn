...
def mech_model(ctype,dose,doseerr,Yld,Ylderr,lmbda,lmbdaerr,DNA,NDIA):
    if dose!=0 and lmbda!=0:
        ...
        Y=ufloat(Yld,Ylderr)*DNA/6
        lmbda=ufloat(lmbda,lmbdaerr)**5**2/NDIA**2
...
def lector_dose_data_FLUKA(dose_data_path):
    ...
    for line in ascii_file:
        if "Z coordinate" in line:
            starting_point = float(line.split(' ')[10]) * 10  # starting point in mm
            ending_point = float(line.split(' ')[13]) * 10  # ending point in mm
            nbins = int(line.split(' ')[16])
...
def fluka_to_data(spectrum_data_path,dose_data_path,database_path,detector_depths,ctype,DNA,NDIA,dose_norm_max):
    ...
    newtablis=open("proc_"+filename,'w')
    summarized_tab=[]
    ...
    for index,detector_depth in enumerate(detector_depths):
        ...
        if sum(spectrum_at_detector_depth)!=0:
            yave=np.average(y, weights=spectrum_at_detector_depth)
            yerrave=np.average(yerr, weights=spectrum_at_detector_depth)
            lave=np.average(l, weights=spectrum_at_detector_depth)
            lerrave=np.average(lerr, weights=spectrum_at_detector_depth)
        else:
            yave = 0
            yerrave = 0
            lave = 0
